...
const APP_OVERLAY_KEYS = ['clone', 'openRepo', 'remoteRepo'] as const
...
export function useAppOverlays() {
  ...
  const setRemoteRepoOpen = useCallback((open: boolean) => {
    setOpen('remoteRepo', open)
  }, [setOpen])
  ...
  const state = useMemo(() => ({
    ...
    remoteRepo: { open: openByKey.remoteRepo },
  }), [openByKey.clone, openByKey.openRepo, openByKey.remoteRepo])
